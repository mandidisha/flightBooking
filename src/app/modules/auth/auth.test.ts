...
import * as mailService from '../../providers/mail/sendgrid';
import server from '../../app';
import error from '../../providers/errors';
import { createToken } from '../../authentication/jwt';
import { generateToken } from '../../authentication/twoFactorAuth';
...
describe('Test "Auth" endpoints', () => {
  ...
  it('Test "POST /auth/register" (Fail test case: Duplicate emails)', async () => {
    ...
    Chai.expect(response.body.details).to.equal(error.DUPLICATE_EMAILS);
  });
  ...
  it('Test "POST /auth/resend-confirmation-email" (Fail test case: No results found)', async () => {
    ...
    Chai.expect(response.body.details).to.equal(error.USER_NOT_FOUND_OR_ACCOUNT_CONFIRMED);
  });
  ...
  it('Test "POST /auth/login" (Fail test case: Account not confirmed)', async () => {
    ...
    Chai.expect(response.body.details).to.equal(error.ACCOUNT_NOT_CONFIRMED);
  });
  ...
  */
  ...
  it('Test "POST /auth/login" (Fail test case: Email not found)', async () => {
    ...
    Chai.expect(response.status).to.equal(401);
    Chai.expect(response.body.details).to.equal(error.USER_NOT_FOUND);
  });
  ...
  it('Test "POST /auth/login" (Fail test case: Invalid password)', async () => {
    ...
    Chai.expect(response.status).to.equal(401);
    Chai.expect(response.body.details).to.equal(error.INVALID_PASSWORD);
  });
 ...
 */
  ...
  it('Test "HEAD /auth/two-factor-auth/verification" (Success test case)', async () => {
    const response = await Chai.request(server)
      .head(`/auth/two-factor-auth/verification?token=${generateToken(twoStepAuthSecretKey)}`)
      .set('authorization', userToken);
